...
testExprEvalWith :: Expr -> SymbolTable -> IO (Either Error (Val, SymbolTable))
testExprEvalWith = testEvalWith evalExpr

testExprEval :: Expr -> IO (Either Error (Val, SymbolTable))
...
spec = do
  describe "리터럴 평가" $ do
    describe "정수 리터럴 평가" $ do
      it "LitInteger 10을 IntegerVal 10으로 평가" $ do
        testExprEval (litInteger 10) `shouldEval` (IntegerVal 10, empty)
    describe "실수 리터럴 평가" $ do
      it "LitReal 10을 RealVal 10으로 평가" $ do
        testExprEval (litReal 10) `shouldEval` (RealVal 10, empty)
    describe "부울 리터럴 평가" $ do
      it "LitBool True를 BoolVal True로 평가" $ do
        testExprEval (litBool True) `shouldEval` (BoolVal True, empty)

  describe "변수 식별자 평가" $ do
    it "심볼 테이블에서 식별자 값 가져오기" $ do
      testExprEvalWith (var "나이") sampleTable
        `shouldEval` (IntegerVal 17, sampleTable)
    it "없는 심볼에 대해서 UnboundSymbol 에러" $ do
      testExprEvalWith (var "사과") sampleTable
  ...
  describe "이항 연산자 평가" $ do
    ...
    it "정수와 함수 더했을 시 타입 에러" $ do
      testExprEvalWith (binaryOp Plus (litInteger 10) (var "십")) sampleTable
  ...
  describe "단항 연산자 평가" $ do
    ...
    it "함수에 양수 단항 연산자 적용 시 타입 에러" $ do
      testExprEvalWith (unaryOp Minus (var "십")) sampleTable
        `shouldEvalError` operateTypeError [FuncType]

  describe "함수 호출 평가" $ do
    it "인자 없는 함수 호출" $ do
      testExprEvalWith (app (var "십") []) sampleTable
        `shouldEval` (IntegerVal 10, sampleTable)
    it "인자가 하나인 함수 호출" $ do
      testExprEvalWith (app (var "늘리기") [litInteger 10]) sampleTable
    ...
    it "호출할 수 없는 대상에 대해 에러" $ do
      testExprEvalWith (app (var "나이") []) sampleTable
        `shouldEvalError` notCallable IntegerType

  describe "대입식 평가" $ do
    it "단순 정수 대입" $ do
      testExprEvalWith (assign "수" (litInteger 10)) sampleTable
    ...
    it "대입식 대입" $ do
      testExprEvalWith (assign "수" (assign "상자" (litInteger 10))) sampleTable
    ...
    it "겹치는 식별자 대입" $ do
      testExprEvalWith (assign "십" (litInteger 10)) sampleTable
