...
누리 0.2.1 버전을 기준으로 언어의 문법에 대한 설명을 기재한 문서입니다.
...
동사로 선언된 함수는 `~(하)고` 의 형식으로 함수끼리 연쇄적으로 호출할 수 있습니다.
...
형용사의 경우 유의어, 반의어를 지정할 수 있습니다.

```python
형용사 [ㄱ]과 [ㄴ]이 같다 (<-> 다르다, = 동일하다):
  [ㄱ] == [ㄴ]
```

위 예시에서 `동일하다`는 `같다`와 같은 정의를 가지며, `다르다`는 `같다`의 결과를 반전하여 반환하는 함수입니다.
